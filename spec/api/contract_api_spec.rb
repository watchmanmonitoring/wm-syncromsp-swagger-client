=begin
#Syncro

#Welcome to the official Syncro API Docs.  To use these docs, you will need an active Syncro account. You can sign up for one here: [Syncro](https://syncromsp.com)  If you already have an active account, fill in your subdomain below and then click \"Authorize\" and fill in your api-key. The key is specific to your user account so it is found on the your user profile page.  Please review the [API License Agreement](https://syncromsp.com/syncromsp-api-license-agreement/) before using our API. By accessing our API, you are agreeing to the API License Agreement.  Please note there is a rate limit of 180 requests per minute per IP address on API Usage.    

OpenAPI spec version: v1
Contact: help@syncromsp.com
Generated by: https://github.com/swagger-api/swagger-codegen.git
Swagger Codegen version: 3.0.25
=end

require 'spec_helper'
require 'json'

# Unit tests for WmSyncromspSwaggerClient::ContractApi
# Automatically generated by swagger-codegen (github.com/swagger-api/swagger-codegen)
# Please update as you see appropriate
describe 'ContractApi' do
  before do
    # run before each test
    @instance = WmSyncromspSwaggerClient::ContractApi.new
  end

  after do
    # run after each test
  end

  describe 'test an instance of ContractApi' do
    it 'should create an instance of ContractApi' do
      expect(@instance).to be_instance_of(WmSyncromspSwaggerClient::ContractApi)
    end
  end

  # unit tests for contracts_get
  # Returns a paginated list of Contracts
  # Required permission: Contracts - List/Search 
  # @param [Hash] opts the optional parameters
  # @option opts [Integer] :page Returns provided page of results, each &#x27;page&#x27; contains 50 results
  # @return [nil]
  describe 'contracts_get test' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
    end
  end

  # unit tests for contracts_id_delete
  # Deletes a Contract by ID
  # Required permission: Contracts - Delete 
  # @param id 
  # @param [Hash] opts the optional parameters
  # @return [nil]
  describe 'contracts_id_delete test' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
    end
  end

  # unit tests for contracts_id_get
  # Retrieves a Contract by ID
  # Required permission: Contracts - Edit 
  # @param id 
  # @param [Hash] opts the optional parameters
  # @return [nil]
  describe 'contracts_id_get test' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
    end
  end

  # unit tests for contracts_id_put
  # Updates an existing Contract by ID
  # Required permission: Contracts - Edit 
  # @param id 
  # @param [Hash] opts the optional parameters
  # @option opts [Body9] :body Contract object that needs to be updated
  # @return [nil]
  describe 'contracts_id_put test' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
    end
  end

  # unit tests for contracts_post
  # Creates a Contract
  # Required permission: Contracts - Edit 
  # @param [Hash] opts the optional parameters
  # @option opts [Body8] :body Contract object that needs to be added
  # @return [nil]
  describe 'contracts_post test' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
    end
  end

end
