  ...
  class Body37
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WmSyncromspSwaggerClient::Body37` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WmSyncromspSwaggerClient::Body37`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'timer_entry_id')
        self.timer_entry_id = attributes[:'timer_entry_id']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          timer_entry_id == o.timer_entry_id &&
          start_at == o.start_at &&
    ...
    def hash
      [timer_entry_id, start_at, duration_minutes, user_id, notes, product_id].hash
    end
